...
export const getUserById = async (req, res) => {
  try {
    ...
    // Basic ID validation to prevent CastError/500
    if (!userId || userId === "null" || userId === "undefined" || userId.length !== 24) {
      return res.status(400).json({ message: "Invalid User ID provided" });
    }

    const user = await User.findById(userId).select("-password -resetOtp -otpExpire -verificationOtp -verificationOtpExpire");
    if (!user) return res.status(404).json({ message: "User not found" });
