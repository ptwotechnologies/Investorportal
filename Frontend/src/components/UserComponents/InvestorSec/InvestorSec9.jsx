      ...
      <section className="mx-auto w-full lg:px-10 py-10 md:py-16">
        ...
        >
          {cards.map((card, idx) => (
            <article
              key={idx}
              className="lg:w-auto w-screen shrink-0 snap-center 
                         mr-4 lg:mr-0 lg:p-6 text-card-foreground"
            >
              {/* Accent Bar */}
              ...
              <div className="mt-10 lg:mt-6 text-[#3C1D3A] px-3">
                <p className="text-2xl font-semibold">
                  {card.priceMain}{" "}
                  <span className="align-baseline text-sm font-normal">
                    {card.priceSub}
                  </span>
                </p>
              ...
              <div className="mt-6 px-3 text-center">
                <button
                  type="button"
                  className="w-full lg:w-[90%] mt-7 h-10 
                             rounded-sm bg-[#002A30] 
                             text-background text-md font-medium 
          ...
          {cards.map((_, index) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                activeIndex === index ? "bg-[#001032] scale-125" : "bg-gray-300"
              }`}
            />
          ))}
        </div>
