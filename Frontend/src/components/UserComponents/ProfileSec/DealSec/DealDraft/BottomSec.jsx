...
const Card = ({ title, children, className = "" }) => (
  <div
    className={` bg-white rounded-2xl lg:px-6 px-3 py-3 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] ${className}`}
  >
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 gap-x-4 gap-y-2">
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <span className=" text-[10px] lg:text-[11px] text-[#1A1A1A]">
                  Clear milestone breakdown
                </span>
              </div>
              {!issues.milestones && (
                <button
                  onClick={() => onRevise("milestone")}
                  className="text-[9px] px-2 py-0.5 bg-[#D8D6F8] text-[#59549F] font-bold rounded hover:opacity-80 transition-opacity"
                >
                  Revise
                </button>
              )}
            ...
            {!issues.milestones && (
              <p className="text-[8px] lg:text-[9px] text-gray-500 mt-0.5 ml-5 leading-tight">
        ...
        <div className="flex flex-col">
          <div className="flex flex-col">
            ...
            {!issues.pricing && (
              <p className="text-[8px] lg:text-[9px] text-gray-500 mt-0.5 ml-5 leading-tight">
                Ensure your total budget is realistic
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col">
          <div className="flex flex-col">
            ...
            {!issues.budgetMatch && (
              <p className="text-[8px] lg:text-[9px] text-gray-500 mt-0.5 ml-5 leading-tight">
                Total budget must equal sum of milestones
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col">
          <div className="flex flex-col">
            ...
            {!issues.timelineMatch && (
              <p className="text-[8px] lg:text-[9px] text-gray-500 mt-0.5 ml-5 leading-tight">
                Total days must equal sum of milestones
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col">
          <div className="flex flex-col">
            ...
            {!issues.scopeDefined && (
              <p className="text-[8px] lg:text-[9px] text-gray-500 mt-0.5 ml-5 leading-tight">
                Define your requirements in detail for the service professional
    ...
    <div className="flex items-start justify-between gap-2">
      <div className="flex lg:gap-4 gap-2 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-[#000000] text-xs lg:text-sm font-semibold truncate">
            {milestone.name}
          </h4>
          <p className="lg:text-xs text-[10px] text-[#000000] mt-1 line-clamp-2 leading-relaxed">
            {milestone.description}
          </p>
          <p className="lg:text-xs text-[10px] text-[#999999] mt-2">
...
const BottomSec = ({
  ...
  const [selectionRequestId, setSelectionRequestId] = useState(
    requestId || null,
  );
  const [selectionProfessionalId, setSelectionProfessionalId] = useState(
    professionalId || null,
  );
  ...
  const [showTopNotificationStrip, setShowTopNotificationStrip] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem(
      `deal_draft_autosave_${currentUser._id}`,
    );
    if (saved) {
      try {
        ...
        if (draftData.selectionRequestId)
          setSelectionRequestId(draftData.selectionRequestId);
        if (draftData.selectionProfessionalId)
          setSelectionProfessionalId(draftData.selectionProfessionalId);

        toast.success("Draft recovered: Continue working on your proposal!", {
          id: "draft-recovered",
        });
  ...
  useEffect(() => {
    ...
    if (hasContent) {
      ...
      localStorage.removeItem(
        `deal_draft_recovery_dismissed_${currentUser._id}`,
      );
    } else {
  ...
  const getProposalIssues = () => {
    const milestoneBudgetSum = milestones.reduce(
      (sum, m) => sum + (Number(m.budget) || 0),
      0,
    );
    const scopeWordCount = getWordCount(description);
    const milestoneDescValid = milestones.every(
      (m) => getWordCount(m.description) >= 150,
    );
    ...
    const milestoneTimelineSum = milestones.reduce(
      (sum, m) => sum + parseDuration(m.duration),
      0,
    );
    ...
    return {
      ...
      budgetMatch:
        totalBudget !== "" && milestoneBudgetSum === Number(totalBudget),
      timelineMatch:
        totalTimeline !== "" && milestoneTimelineSum === totalTimelineNum,
    ...
    const fetchEligibleRequests = async () => {
      ...
      try {
        ...
        if (eligible.length === 1) {
          ...
          setSelectionProfessionalId(
            req.acceptedProvider?._id ||
              req.acceptedProvider ||
              req.userId?._id ||
              req.userId,
          );
        }
  ...
  const handleSaveMilestone = (silent = false) => {
    ...
    if (activeView === "addMilestone") {
      ...
      updatedMilestones = milestones.map((m) =>
        m.id === tempMilestone.id ? tempMilestone : m,
      );
    }
  ...
  const handleSaveScope = (silent = false) => {
    ...
    if (newScopeInput.trim()) {
      if (finalItems.length >= 4) {
        if (!silent)
          toast.error(
            "You can add a maximum of 4 scope titles. Please remove one to add another.",
          );
  ...
  const handleAddTempScope = () => {
    const currentCount = tempScopeItems.filter(
      (item) => item.trim() !== "",
    ).length;
    if (currentCount >= 4) {
  ...
  const handleSubmitDraft = async () => {
    ...
    try {
      ...
      const dealData = {
        ...
        totalAmount:
          totalBudget ||
          currentMilestones.reduce((sum, m) => sum + Number(m.budget || 0), 0),
        ...
        <div className="flex-1 p-6 flex flex-col items-center justify-center min-h-[400px]">
          <div className="w-full max-w-2xl bg-white rounded-3xl p-8 shadow-[0px_0px_24px_rgba(0,0,0,0.1)] border border-gray-100">
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold text-[#001032]">
                Select a Request
              </h2>
              <p className="text-gray-500 mt-2">
                Which project would you like to create a deal for?
              </p>
            </div>
            ...
            {isFetchingRequests ? (
              ...
              <div className="grid gap-4">
                {eligibleRequests.map((req) => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <h4 className="font-bold text-[#001032] group-hover:text-[#59549F]">
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500 mt-2 max-w-sm mx-auto">
                  You need to have an accepted professional on a request before
                  you can create a deal draft.
                </p>
                <button
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="flex flex-col lg:flex-row gap-2 px-1 lg:px-4 lg:py-1 bg-[#FDFDFF]">
        ...
        <div
          className={`flex-1 min-h-0 space-y-4 max-h-[620px] overflow-y-auto p-2 scrollbar-hide mb-2 ${activeView !== "none" ? "hidden lg:block" : "block"}`}
        >
          ...
          <Card title="Scope of Work">
            <ul className="flex flex-col gap-3 mb-2">
              {[
                ...(activeView === "scope"
                  ? tempScopeItems.filter((it) => it.trim() !== "")
                  : scopeItems),
                ...(activeView === "scope" && newScopeInput.trim()
                  ? [newScopeInput.trim()]
                  : []),
              ].map((item, i) => {
                if (i >= 2) return null;
                return (
                  <li
                    key={i}
                    className="text-[#000000] text-sm flex items-start gap-2"
                  >
              ...
              {(activeView === "scope"
                ? [...tempScopeItems, newScopeInput.trim()].filter(Boolean)
                : scopeItems
              ).length === 0 && (
              ...
              {(activeView === "scope"
                ...
                <li className="text-gray-400 text-[10px] italic pl-4">
                  +{" "}
                  {(activeView === "scope"
                    ? [...tempScopeItems, newScopeInput.trim()].filter(Boolean)
                    : scopeItems
          ...
          <Card title="Total Budget">
            ...
            {milestones.length > 0 &&
              totalBudget !==
                milestones
                  .reduce((acc, m) => acc + (Number(m.budget) || 0), 0)
                  .toString() && (
                <p className="text-[10px] text-red-500 mt-1 text-center">
                  Note: Total doesn't match sum of milestones (
                  {milestones.reduce(
                    (acc, m) => acc + (Number(m.budget) || 0),
                    0,
                  )}
                  )
                </p>
        ...
        <div
          className={`lg:w-[450px] xl:w-[550px] flex flex-col flex-1 min-h-0 px-2 lg:px-0 ${activeView === "none" ? "hidden lg:block" : "block"}`}
        >
          {/* Editor Wrapper */}
          <div
            className={`flex flex-col transition-all duration-300 ${activeView === "none" ? "h-[620px]" : "lg:h-[568px] h-[490px]"}`}
          >
            {showImproveCard && (
              <ImproveProposalCard
                issues={issues}
                onRevise={(section) => {
                  if (section === "scope") {
                    ...
                    const milestoneWithIssue = milestones.find(
                      (m) => getWordCount(m.description) < 150,
                    );
            ...
            <div
              className={`bg-white rounded-2xl lg:px-4 px-3 pt-4 pb-4 mt-2 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] border border-gray-100 flex flex-col relative transition-all duration-300 flex-1 min-h-0 overflow-y-auto scrollbar-hide`}
            >
              {activeView === "none" && (
                <div className="flex-1 flex flex-col min-h-0 items-center justify-center text-center p-10">
                  <div className="flex flex-col items-center gap-4 p-8 text-center border border-gray-300 shadow-[0_4px_16px_rgba(0,0,0,0.15)] rounded-md bg-white w-full max-w-sm">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 italic">
                        Click "Add Details" on any section to start editing your
                        draft.
                      </p>
                    </div>
                  </div>
              ...
              {activeView === "scope" && (
                <div className="flex-1 flex flex-col min-h-0 h-full">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-[#001032]">
                      Scope of Work
                    </h3>
                  </div>
                      ...
                      {tempScopeItems.map((item, index) => (
                        <div key={index} className="flex items-center gap-3">
                          <input
                            ...
                            placeholder={
                              index === 0
                                ? "Develop a mobile app with core features"
                                : index === 1
                                  ? "Create UI/UX designs for all screens"
                                  : "Add scope item detail"
                            }
                            onChange={(e) => {
                              const updated = [...tempScopeItems];
                      ...
                      {tempScopeItems.length < 4 && (
                        <div className="flex items-center gap-3">
                          <input
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" && handleAddTempScope()
                            }
                            className="flex-1 px-5 py-2.5 bg-white border border-gray-100 rounded-lg text-xs focus:border-[#59549F] outline-none transition-all shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)]"
                          />
                          <button
                  ...
                  <div className="flex-1 flex flex-col min-h-0 mt-2 ">
                    <div className="flex items-center justify-between mb-2 px-1">
                      <h4 className="text-lg font-medium text-[#000000]">
                        Description
                      </h4>
                      {!issues.scopeDefined && (
                  ...
                  <div className="flex gap-4 mt-4 ">
                    ...
                    <button
                      onClick={() => setActiveView("none")}
                      className="flex-1 py-1 bg-white border border-[#000000]/20 text-gray-600 font-semibold rounded-lg hover:bg-gray-50 shadow-[inset_0px_0_12px_#00000040]"
                    >
                      Cancel
                    </button>
                  </div>
              ...
              {activeView.includes("Milestone") && tempMilestone && (
                <div className="flex-1 flex flex-col min-h-0 h-full ">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3 ">
                      ...
                      <h3 className="text-lg font-medium text-[#1A1A1A]">
                        {tempMilestone.name || "New Milestone"}
                      </h3>
                    </div>
                    {activeView === "editMilestone" && (
                      <button
                        onClick={() => handleRemoveMilestone(tempMilestone.id)}
                  ...
                  <div className="space-y-4 flex-1 pr-2">
                    {/* Name */}
                    <div>
                      <label className="text-sm font-medium text-[#000000] block mb-2">
                        Milestone Name
                      </label>
                      <input
                        type="text"
                    ...
                    {/* Milestone Budget */}
                    <div
                      className={`bg-white border ${!issues.budgetMatch ? "border-red-300" : "border-gray-100"} rounded-2xl px-3 lg:px-6 py-3 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)]`}
                    >
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-sm font-medium text-[#000000]">
                          Budget - {tempMilestone.name || "Milestone"}
                        </h4>
                        {!issues.budgetMatch && (
                      ...
                      <div className="flex gap-3 mb-2">
                        <div className="w-[150px] px-3 py-2 bg-[#FDFDFF] border border-gray-100 rounded-lg text-[10px] text-gray-400 whitespace-nowrap text-center shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)]">
                          INR - Indian Rupees
                        </div>
                        <input
                          type="number"
                    ...
                    {/* Milestone Timeline */}
                    <div
                      className={`bg-white border ${!issues.timelineMatch ? "border-red-300" : "border-gray-100"} rounded-2xl lg:px-6 px-3 py-3 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)]`}
                    >
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-sm font-medium text-[#000000]">
                          Timeline - {tempMilestone.name || "Milestone"}
                        </h4>
                        {!issues.timelineMatch && (
                          <span className="text-[10px] text-red-500">
                            Duration match required
                          </span>
                        )}
                      </div>
                      <div className="flex gap-3 mb-2 ">
                    ...
                    <div className=" ">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-sm font-medium text-[#000000]">
                          Scope of work in {tempMilestone.name || "milestone"}
                        </h4>
                        {getWordCount(tempMilestone.description) < 150 &&
                          submitAttempted && (
                            <span className="text-[10px] text-red-500 font-medium">
                              Min. 150 words
                            </span>
                    ...
                    <div className="flex gap-4 mt-4 mb-4">
                      <button
                        ...
                        className="flex-1 lg:py-1 py-1 bg-[#D8D6F8] text-[#59549F] font-semibold rounded-lg hover:opacity-90 shadow-[inset_0px_0_12px_#00000040]"
                      >
                        {activeView === "addMilestone"
                          ? "Add Milestone"
                          : "Save Changes"}
                      ...
                      <button
                        onClick={() => setActiveView("none")}
                        className="flex-1 lg:py-1 py-1 bg-white border border-[#59549F]/20 text-gray-600 font-semibold rounded-lg hover:bg-gray-50 shadow-[inset_0px_0_12px_#00000040]"
                      >
                        Cancel
                      </button>
                    </div>
