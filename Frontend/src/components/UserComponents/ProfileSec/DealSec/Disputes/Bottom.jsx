...
const Bottom = ({
  ...
  useEffect(() => {
    if (initialDealId && deals.length > 0) {
      const deal = deals.find(
        (d) => d && String(d._id) === String(initialDealId),
      );
      if (deal) {
        setSelectedDeal(deal);
        if (initialMilestoneId) {
          const m = deal.milestones?.find(
            (ms) => String(ms._id) === String(initialMilestoneId),
          );
          setFormData((prev) => ({
            ...prev,
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${serverUrl}/api/disputes/upload-evidence`,
        uploadData,
        {
          headers: {
  ...
  const ProjectCard = ({ proj, isSelected, onSelect }) => {
    ...
    const professional =
      typeof proj.professionalId === "object" ? proj.professionalId : {};
    const ownerName = professional.businessDetails
      ? `${professional.businessDetails.firstName} ${professional.businessDetails.lastName}`
      : "N/A";
        ...
        <div className="grid grid-cols-3 gap-2 lg:gap-2 mb-4 items-start w-full">
          ...
          <div className="flex flex-col -mt-1 overflow-hidden">
            <p className="text-[13px] lg:text-sm text-[#000000] truncate">
              {proj.requestId?.service ||
                deals.find((d) => d && d._id === proj._id)?.requestId
                  ?.service ||
                "Project Deal"}
            </p>
          </div>
          ...
          <div className="col-span-3 mt-1">
            <p className="text-[13px] lg:text-sm text-[#000000] opacity-70">
              {ownerName}
            </p>
          </div>
        </div>
  ...
  const DisputeItem = ({ dispute, deal }) => (
    <div className="bg-white rounded-xl lg:rounded-2xl p-3 mt-1 shadow-[0px_0px_12px_rgba(0,0,0,0.25)] border border-gray-50 mb-6 last:mb-0">
      <div className="flex items-center justify-between mb-4 px-1">
        <h4 className="lg:text-[15px] text-[11px] font-medium text-[#001032]">
      ...
      <div className="bg-[#F9FAFB]/50 rounded-xl border border-gray-100 lg:p-3 p-2 lg:px-6 shadow-[0px_0px_12px_rgba(0,0,0,0.25)] space-y-3">
        <h5 className="text-[14px] lg:text-base font-medium text-[#001032]">
          {deal?.milestones?.find((m) => m && m._id === dispute.milestoneId)
            ?.title || "Milestone 1"}
        </h5>
        <p className="text-[10px] text-gray-400 -mt-2 truncate">
          {deal?.requestId?.service ||
            deals.find(
              (d) =>
                d &&
                d._id === (deal?._id || dispute.dealId?._id || dispute.dealId),
            )?.requestId?.service ||
            "Project Deal"}
        </p>
        <div className="space-y-2">
          <p className="text-[12px] lg:text-sm text-gray-500 leading-relaxed line-clamp-2">
            <span className="text-[#59549F] font-medium">Reason</span> –{" "}
            {dispute.reason}
          </p>
  ...
  const displayDeals = disputes
    .map((d) => d.dealId)
    .filter((v, i, a) => v && a.findIndex((t) => t && t._id === v._id) === i);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-2 lg:px-4 lg:py-1 bg-[#FDFDFF] flex-1 min-h-0 overflow-hidden">
      ...
      >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1 w-full">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-1 lg:gap-2 mt-1">
                  <p className="text-xl lg:text-2xl font-bold text-[#001032] leading-none">
                    {
                      disputes.filter((d) => d && d.status === "Resolved")
                        .length
                    }
                  </p>
        ...
        <h2 className="text-xl font-medium text-[#000000] px-3 shrink-0">
          Case List
        </h2>
            ...
            <div className="flex flex-col items-center gap-4 lg:p-8 p-5 text-center border border-gray-300 shadow-[0_4px_16px_rgba(0,0,0,0.15)] rounded-md bg-white w-[90%] lg:w-auto max-w-sm mx-auto lg:my-10">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Your project disputes and case history will appear here.
                </p>
              </div>
            </div>
          )}
      ...
      {/* ── Right Column ── */}
      <div
        className={`w-full lg:w-[450px] xl:w-[550px] h-[calc(100vh-140px)] lg:h-full flex flex-col flex-1 min-h-0 overflow-hidden ${selectedDeal || selectedDispute ? "flex" : "hidden lg:flex"}`}
      >
        {/* Header */}
        ...
        <div
          className={`flex-1 flex flex-col min-h-0 overflow-hidden m-2 rounded-2xl relative ${!selectedDispute && !isCreateMode ? "bg-transparent border-none" : "bg-white shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] border border-gray-100"}`}
        >
          <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide relative pb-32 lg:pb-0">
            {!selectedDeal && !selectedDispute ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-10 opacity-50">
                ...
                <p className="text-sm text-gray-400 mt-1 italic">
                  Select a case from the list to view active disputes.
                </p>
              </div>
              ...
              <div className="p-3 lg:p-4 space-y-4">
                <div className="bg-white rounded-2xl border border-gray-100 lg:p-5 p-3 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] space-y-3">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const m = selectedDeal.milestones?.find(
                        (ms) => String(ms._id || ms.id) === String(mId),
                      );
                      setFormData({
                        ...formData,
                        milestoneId: mId,
                ...
                <div className="space-y-3">
                  <h5 className="text-sm font-semibold text-[#001032] px-2">
                    Reason
                  </h5>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] space-y-4">
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">
                    Upload the evidence in jpg, pdf and docx format
                  </p>
                  <div className="grid grid-cols-2 gap-4">
                    {formData.evidence?.map((url, i) => (
                      <div
                        key={i}
                        className="aspect-[4/3] relative rounded-2xl overflow-hidden shadow-sm border border-gray-100 bg-gray-50"
                      >
                        <img
                          src={getImageUrl(url)}
                          alt="evidence"
                    ...
                    {formData.evidence?.length < 4 && (
                      ...
                      >
                        {isUploading ? (
                          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#59549F]"></div>
                        ) : (
                          <FiPlus size={32} />
                        )}
                      </div>
              ...
              <div className="p-4 lg:p-6 space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 lg:p-8 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] space-y-4">
                  <h5 className="text-sm lg:text-base font-bold text-[#001032]">
                    {selectedDeal?.milestones?.find(
                      (m) => m._id === selectedDispute.milestoneId,
                    )?.title || "Milestone 1"}
                  </h5>
                  <div className="space-y-3">
                    ...
                    <p className="text-[12px] lg:text-sm text-gray-500 font-medium">
                      <span className="text-[#59549F] font-bold">Amount</span> –
                      Rs {selectedDispute.amount}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 lg:p-8 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] space-y-4">
                  <div>
                    <h4 className="text-sm lg:text-base font-bold text-[#001032]">
                      Evidence Upload
                    </h4>
                    <p className="text-[12px] lg:text-xs text-gray-400 mt-1">
                      Upload the evidence in jpg, pdf and docx format
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {selectedDispute.evidence
                      ...
                      .map((url, i) => (
                        <div
                          key={i}
                          className="aspect-[4/3] bg-white border border-gray-100 rounded-2xl shadow-[0px_0px_10px_0px_rgba(0,0,0,0.1)] overflow-hidden"
                        >
                          <img
                            src={getImageUrl(url)}
                            alt="evidence"
              ...
              <div className="px-2 space-y-4">
                {disputes
                  .filter((d) => d?.dealId?._id === selectedDeal?._id)
                  .map((dispute) => (
                    <DisputeItem
                      key={dispute._id}
                      dispute={dispute}
                ...
                {disputes.filter((d) => d?.dealId?._id === selectedDeal?._id)
                  .length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center p-10 opacity-40">
                    ...
                    <p className="text-sm text-gray-500">
                      There are no active disputes for this project.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 lg:sticky lg:bottom-0 z-50 lg:z-20 px-2 lg:py-2 lg:px-2 bg-[#FDFDFF]/60 backdrop-blur-md lg:bg-transparent shadow-none lg:shadow-none space-y-3">
            {isCreateMode && selectedDeal && !selectedDispute && (
              <>
                ...
                <button
                  onClick={() =>
                    navigate("/deal/communication", {
                      state: { dealId: selectedDeal._id },
                    })
                  }
