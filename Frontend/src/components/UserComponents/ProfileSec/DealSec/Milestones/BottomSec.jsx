...
const BottomSec = ({
  ...
  const handleUpdateMilestone = async (milestoneId, updates) => {
    ...
    try {
      ...
      const updatedM = updatedMilestones.find(
  ...
  const ProjectCard = ({ deal, isSelected, onSelect }) => {
    ...
    const isStartupOwner =
      String(deal.startupId?._id || deal.startupId) === String(currentUserId);
    ...
    const paidDates = deal.milestones
      ?.filter((m) => m.paidAt)
      .map((m) => new Date(m.paidAt));
    const startDate =
      paidDates?.length > 0
        ? new Date(Math.min(...paidDates)).toLocaleDateString("en-IN", {
            day: "numeric",
            month: "short",
            year: "numeric",
          })
  ...
  const MilestoneCard = ({ ms, isSelected, onSelect }) => {
    ...
    const isStartupOwner =
      String(selectedDeal.startupId?._id || selectedDeal.startupId) ===
      String(currentUserId);
    const displayUser = isStartupOwner
      ? selectedDeal.professionalId
      : selectedDeal.startupId;
    ...
    return (
      <div
        className={`bg-white rounded-2xl p-3 lg:p-6 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] border-2 transition-all ${isSelected ? "border-[#D8D6F8]" : "border-transparent"}`}
      >
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="space-y-2 overflow-hidden">
            <h3 className="text-sm lg:text-sm font-semibold text-[#000000] truncate">
              {ms.title}
            </h3>
          ...
          <div className="space-y-2 text-center">
            <h3 className="text-sm lg:text-sm font-semibold text-[#000000]">
              Due Date
            </h3>
            <p className="text-sm lg:text-sm text-gray-500 font-medium">
              {ms.duration || "20 Days"}
            </p>
          </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-2 lg:px-4 lg:py-1 bg-[#FDFDFF] flex-1 min-h-0 overflow-hidden">
      ...
      <div
        className={`flex-1 flex flex-col min-h-0 py-2 gap-6 overflow-y-auto scrollbar-hide ${selectedDeal ? "hidden lg:flex" : "flex"}`}
      >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-1 lg:gap-2 mt-1">
                  <p className="text-xl lg:text-2xl font-bold text-[#001032] leading-none">
                    {deals.reduce(
                      (acc, d) =>
                        acc +
                        (d.milestones?.filter((m) => m.status === "Paid")
                          .length || 0),
                      0,
                    )}
                  </p>
          ...
          <div className="bg-gradient-to-br from-[#FDF5E6] to-[#FFFFFF] shadow-[0px_0px_12px_0px_rgba(0,0,0,0.50)] px-3 py-4 lg:p-4 rounded-2xl flex flex-col justify-between min-h-[100px] border border-orange-50">
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1 w-full">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-1 lg:gap-2 mt-1">
                  <p className="text-xl lg:text-2xl font-bold text-[#001032] leading-none">
                    Rs {deals.reduce((acc, d) => acc + (d.totalAmount || 0), 0)}
                  </p>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-1 lg:gap-2 mt-1">
                  <p className="text-xl lg:text-2xl font-bold text-[#001032] leading-none">
                    {deals.reduce(
                      (acc, d) =>
                        acc +
                        (d.milestones?.filter((m) => m.status === "Completed")
                          .length || 0),
                      0,
                    )}
                  </p>
        ...
        <h2 className="text-xl font-medium text-[#000000] px-1 shrink-0">
          Active Deals
        </h2>
      ...
      <div
        className={`w-full lg:w-[450px] xl:w-[550px] h-[75vh] lg:h-full flex flex-col flex-1 min-h-0 overflow-hidden ${!selectedDeal ? "hidden lg:flex" : "flex"}`}
      >
        {/* Header */}
        {selectedDeal && (
          <div
            className={`flex items-center gap-3 py-2 px-4 shrink-0 ${selectedDeal ? "lg:hidden" : ""}`}
          >
            <button
              onClick={() => {
            ...
            {!selectedDeal ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-10 opacity-50">
                ...
                <p className="text-sm text-gray-400 mt-1 italic">
                  Select a project from the left to view milestones.
                </p>
              </div>
            ) : (
              <div className="p-2 lg:p-4 space-y-6 min-h-[500px]">
                {selectedMilestone ? (
                  ...
                  <div className="bg-white p-2 flex flex-col space-y-6">
                    ...
                    <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-[0px_0px_12px_rgba(0,0,0,0.25)] space-y-3">
                      <h4 className="text-sm font-semibold text-[#000000]">
                        Budget - {selectedMilestone.title}
                      </h4>
                    ...
                    <div className="space-y-3">
                      <h4 className="text-sm font-semibold text-[#000000]">
                        Scope of work in {selectedMilestone.title.toLowerCase()}
                      </h4>
                      <div className="w-full bg-white border border-gray-100 rounded-xl p-3 lg:p-4 shadow-[0px_0px_12px_rgba(0,0,0,0.25)] h-[150px] overflow-y-auto scrollbar-hide">
                        <p className="text-xs text-gray-500 leading-relaxed text-justify">
                          {selectedMilestone.description ||
                            "No specific scope provided for this milestone yet. These Terms and Conditions govern the access to and use of the website and collaboration portal."}
                        </p>
                      </div>
                    </div>
                          ...
                          <div className="bg-[#EEECFD] rounded-xl lg:rounded-2xl p-1 py-3 lg:p-3 flex flex-col items-center justify-center space-y-1 lg:space-y-2 shadow-[inset_0px_0px_8px_0px_rgba(0,0,0,0.15)] text-center">
                            ...
                            <p className="text-[13px] lg:text-lg text-[#59549F] font-bold">
                              ...
                              {String(
                                ...
                                ? milestoneBreakdown.totalPayable
                                : milestoneBreakdown.netRevenue}
                            </p>
                          </div>
                        </div>
                  ...
                  /* ═══ Add Milestone Form ═══ */
                  String(
                    selectedDeal.startupId?._id || selectedDeal.startupId,
                  ) !== String(currentUserId) ? (
                    <div className="flex flex-col items-center justify-center h-full text-center p-10 opacity-60">
                      ...
                      <p className="text-sm font-medium italic">
                        Only the project creator (Buyer) can add new milestones.
                      </p>
                      <button
                        onClick={() => setActiveView("none")}
                        className="mt-4 text-[#59549F] text-xs font-bold underline"
                      >
                        Go Back
                      </button>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-3 lg:p-6 shadow-[0px_0px_12px_0px_rgba(0,0,0,0.25)] border border-gray-100 flex flex-col space-y-6">
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <label className=" text-[13px] lg:text-[10px] font-semibold text-gray-400 tracking-widest px-1">
                            Milestone Name
                          </label>
                          <input
                            type="text"
        ...
        <div className="shrink-0 p-2 lg:p-0 sticky bottom-0 lg:relative bg-[#FDFDFF] z-50">
          ...
          {selectedDeal && selectedMilestone && (
            <div className="lg:px-6 pb-2 pt-1 bg-[#FDFDFF] border-t border-gray-100 space-y-4 rounded-lg lg:rounded-none shadow-[0px_-4px_12px_rgba(0,0,0,0.05)] lg:shadow-none">
              {String(
                selectedDeal.professionalId?._id || selectedDeal.professionalId,
              ) === String(currentUserId) ? (
                <div className="space-y-4">
                  {showExtensionInput &&
                    selectedMilestone.status !== "Completed" && (
                      <div className="p-4 bg-gray-50 rounded-xl space-y-3 shadow-inner mb-2">
                        <p className="text-[10px] font-bold text-gray-400 uppercase text-center tracking-wider">
                          Extra days needed?
                        </p>
                        <div className="flex gap-2">
                          <input
                          ...
                          <button
                            onClick={() =>
                              handleRequestExtension(selectedMilestone._id)
                            }
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      disabled={
                        isSubmitting || selectedMilestone.status === "Completed"
                      }
                  ...
                  {selectedMilestone.extensionRequest?.status === "Pending" &&
                    selectedMilestone.status !== "Completed" && (
                      <div className="p-3 bg-[#FFF9C4] border border-[#FBC02D] rounded-xl text-center text-[10px] text-[#827717] font-bold">
                        Time Extension Pending (
                        {selectedMilestone.extensionRequest.days} Days)
                      </div>
                    )}
                </div>
              ) : (
                <div className="space-y-4">
                  {selectedMilestone.status === "Completed" ? (
                    ...
                    <button
                      disabled
                      className="w-full py-2 bg-[#D8D6F8] text-[#59549F] rounded-lg lg:rounded-xl text-sm font-bold shadow-[inset_0px_0px_12px_0px_rgba(0,0,0,0.1)] opacity-80 cursor-default tracking-wider"
                    >
                  ...
                  {selectedMilestone.extensionRequest?.status === "Pending" &&
                    selectedMilestone.status !== "Completed" && (
                      <div className="p-4 bg-[#FFF9C4] rounded-2xl space-y-3 border border-[#FBC02D]">
                        <p className="text-sm font-bold text-[#827717]">
                          ...
                          <button
                            onClick={() =>
                              handleApproveExtension(
                                ...
                                selectedMilestone.extensionRequest.days,
                                selectedMilestone.duration,
                              )
                            }
                            className="flex-1 py-2 bg-[#2D6A4F] text-white font-bold rounded-lg text-sm"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() =>
                              handleDisapproveExtension(selectedMilestone._id)
                            }
                            className="flex-1 py-2 bg-[#B91C1C] text-white font-bold rounded-lg text-sm"
                          >
                            Disapprove
                          </button>
                        </div>
                  ...
                  {selectedMilestone.isDisapproved && (
                    <button
                      ...
                      className="w-full py-3 bg-[#B91C1C] text-white font-bold rounded-xl shadow-lg text-sm"
                    >
                      Raise a Dispute
                    </button>
                  )}
