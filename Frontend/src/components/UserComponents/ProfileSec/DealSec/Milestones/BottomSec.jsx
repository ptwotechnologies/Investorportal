      ...
      <div className={`w-full lg:w-[450px] xl:w-[550px] h-full flex flex-col overflow-hidden ${!selectedDeal ? 'hidden lg:flex' : 'flex'}`}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {selectedDeal && selectedMilestone && (
            <div className="pt-4  px-2 bg-[#FDFDFF] border-t border-gray-100 shrink-0 space-y-4">
               {String(selectedDeal.professionalId?._id || selectedDeal.professionalId) === String(currentUserId) ? (
                 ...
                 <div className="space-y-4">
                    {selectedMilestone.status === "Completed" ? (
                      ...
                      <button disabled className="w-full py-2 bg-[#D8D6F8] text-[#59549F] rounded-xl text-sm font-bold shadow-[inset_0px_0px_12px_0px_rgba(0,0,0,0.1)] opacity-80 cursor-default  tracking-wider">
                        Processing...
                      </button>
                    )}
                    {selectedMilestone.extensionRequest?.status === "Pending" && (
          ...
          {activeView === 'addMilestone' && selectedDeal && String(selectedDeal.startupId?._id || selectedDeal.startupId) === String(currentUserId) && (
            <div className="pt-2 pb-8 px-2 bg-[#FDFDFF] border-t border-gray-100 shrink-0 flex gap-4">
               <button 
