                ...
                <div className="grid flex-1 auto-rows-min gap-6 text-[#001032] text-xl px-7">
                  ...
                  <div id='top'>
                    <ul className='flex flex-col gap-2'>
                      ...
                      <li className="cursor-pointer" onClick={handleNotificationClick}>Notification</li>
                    </ul>

                    {showNotifications && (
                      <div className="absolute left-1/2 -translate-x-1/2 top-46 w-[90%] max-h-[60vh] bg-white border border-gray-300 shadow-lg rounded-md z-50 p-4 overflow-y-auto">
                        ...
                        <div className="flex flex-col gap-2 px-2 py-3">
                          {notifications.length === 0 ? (
                            ...
                            notifications.map((n) => {
                              ...
                              return (
                                <div key={n._id} className="border p-2 flex items-start gap-4 rounded-lg w-full">
                                  <div className="w-10 h-10 rounded-full border flex items-center justify-center shrink-0"></div>
                                  <div className="flex-1 min-w-0">
                                    <p className="font-semibold">{n.title}</p>
                                    <p className={`text-sm text-gray-600 transition-all duration-300 ${!isExpanded ? 'line-clamp-2' : ''}`}>{n.message}</p>
                                    {n.message.length > 100 && (
                                      <button className="text-blue-600 text-sm mt-1" onClick={() => toggleExpanded(n._id)}>
