...
const DesigningContent = () => {
  ...
  return (
   <section className="mx-auto w-full md:py-16 px-3 lg:px-0">
                    
            
                    <div
                      ref={scrollRef}
                      onScroll={handleScroll}
                      className="lg:grid lg:gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 flex overflow-x-auto snap-x snap-mandatory scrollbar-hide"
                    >
                      {cards.map((card, idx) => (
                        <article key={idx} className=" w-full  my-4 lg:my-0   lg:p-6 text-card-foreground shrink-0 lg:shrink snap-center">
                          <h1 className='border-2 border-[#000000] text-[#001032] rounded-full p-1 px-6 text-3xl font-bold text-center w-[60%] mx-auto mb-6 lg:mb-13'>{card.accent}</h1>
                          {/* accent bar */}
